...
GITREPO=${GITREPO:-"git://github.com/cubieboard2-b2g/b2g-manifest"}
...
case "$1" in
"cubieboard2")
	echo DEVICE=sugar-cubieboard2 >> .tmp-config &&
	echo PRODUCT_NAME=sugar-cubieboard2 >> .tmp-config &&
	echo LUNCH=sugar-cubieboard2-eng >> .tmp-config &&
	repo_sync $1
	;;
